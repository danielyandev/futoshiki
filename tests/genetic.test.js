...
import { solveWithGeneticAlgorithm } from '../src/solvers/genetic.js'
...
function calculateAverages(statsArray) {
  ...
  return {
    ...
    iterations: totalIterations / size,
...
function solveWithSettings(settings) {
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    const { solved, stats } = solveWithGeneticAlgorithm(board)
    if (solved) {
      allStats.push(stats)
    } else {
      failures++
...
describe('Genetic solver', function () {
